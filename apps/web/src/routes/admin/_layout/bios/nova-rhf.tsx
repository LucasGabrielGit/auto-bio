...
function NovaBioRHFComponent() {
  ...
  const linksTemplates = [
    { platform: 'linkedin', label: 'LinkedIn', icon: <Linkedin className="h-4 w-4" /> },
    { platform: 'github', label: 'GitHub', icon: <Github className="h-4 w-4" /> },
    { platform: 'twitter', label: 'Twitter', icon: <Twitter className="h-4 w-4" /> },
    { platform: 'instagram', label: 'Instagram', icon: <Instagram className="h-4 w-4" /> },
    { platform: 'facebook', label: 'Facebook', icon: <Facebook className="h-4 w-4" /> },
    { platform: 'youtube', label: 'YouTube', icon: <Youtube className="h-4 w-4" /> },
    { platform: 'website', label: 'Website', icon: <Globe className="h-4 w-4" /> },
    { platform: 'email', label: 'Email', icon: <Mail className="h-4 w-4" /> },
    { platform: 'pinterest', label: 'Pinterest', icon: <Camera className="h-4 w-4" /> },
    { platform: 'tiktok', label: 'TikTok', icon: <Heart className="h-4 w-4" /> },
    { platform: 'onlyfans', label: 'OnlyFans', icon: <Heart className="h-4 w-4" /> },
    { platform: 'other', label: 'Outro', icon: <LinkIcon className="h-4 w-4" /> }
  ]
  ...
  const addSocialLink = () => {
    ...
    const availableTemplates = linksTemplates.filter(template =>
      !usedPlatforms.includes(template.platform)
    )
  ...
  const generateWithAI = async () => {
    ...
    if (!title || !style) {
      toast.error('Por favor, preencha o título e selecione um estilo antes de gerar conteúdo')
      return
    }
        ...
        <div className="lg:col-span-2 space-y-6 relative">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  Se deixado vazio, será gerado automaticamente baseado no título
                </p>
              </div>
                      ...
                      <div className="p-3 border rounded-lg bg-primary/5 border-primary">
                        <div className="flex items-center space-x-3">
                          <div className="p-2 bg-primary/10 rounded-lg">
                            {bioTemplates.find(t => t.id === watch('template'))?.icon}
                          </div>
                          <div>
                            <h3 className="font-medium">
                              {bioTemplates.find(t => t.id === watch('template'))?.name}
                            </h3>
                            <p className="text-sm text-muted-foreground">
                              {bioTemplates.find(t => t.id === watch('template'))?.description}
                            </p>
                          </div>
                        </div>
                      </div>
                  ...
                  <Dialog open={templateDialogOpen} onOpenChange={setTemplateDialogOpen}>
                    ...
                    <DialogContent className="max-w-4xl max-h-[80vh] w-152 overflow-y-auto">
                      <DialogHeader>
                        ...
                        <DialogDescription>
                          Selecione o template que melhor se adequa ao seu perfil.
                          {userPlan === 'free' && (
                            <span className="text-primary"> Upgrade para Premium para acessar todos os templates.</span>
                          )}
                        </DialogDescription>
                      </DialogHeader>
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
                        {bioTemplates.map((template) => {
                          const isAvailable = template.planRequired === 'free' || userPlan === 'premium'
                          return (
                            <HoverCard key={template.id}>
                              ...
                              <HoverCardContent className="w-96 p-4">
                                <div className="space-y-4">
                                  ...
                                  <div className="space-y-2">
                                    <h5 className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Preview do Layout</h5>
                                  ...
                                  <div className="text-xs text-muted-foreground border-t pt-2">
                                    <strong>Características:</strong> {template.preview}
                                  </div>

                                  {template.planRequired === 'premium' && userPlan === 'free' && (
                                    <div className="p-3 bg-linear-to-r from-amber-50 to-orange-50 border border-amber-200 rounded-lg">
                                      <div className="flex items-center space-x-2">
                                        <Crown className="h-4 w-4 text-amber-600" />
                                        <span className="text-sm font-medium text-amber-800">Template Premium</span>
                                      </div>
                                      <p className="text-xs text-amber-700 mt-1">
                                        Faça upgrade para acessar este template exclusivo
                                      </p>
                                    </div>
                                  )}
                                </div>
          ...
          <Card>
            ...
            <CardContent>
                  ...
                  >
                    <label className="cursor-pointer">
                      <input
                        type="radio"
                        value={style.value}
                        {...register('style', { required: 'Selecione um estilo' })}
                        className="sr-only"
                      />
                      <div className="flex items-start space-x-3">
                        <div className="p-2 bg-primary/10 rounded-lg">
              ...
              {errors.style && (
                <p className="text-sm text-destructive mt-2">{errors.style.message}</p>
              )}
            </CardContent>
          </Card>
            ...
            <CardContent>
              <Textarea
                ...
                {...register('content', {
                  ...
                  minLength: { value: 10, message: 'Conteúdo deve ter ao menos 10 caracteres' }
                })}
              />
              {errors.content && (
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              {fields.map((field, index) => {
                const selectedTemplate = linksTemplates.find(t => t.platform === field.platform)
                return (
                  <div key={field.id} className="flex space-x-2 items-center">
                    <div>
                      <Select
                        ...
                        onValueChange={(value) => {
                          const template = linksTemplates.find(t => t.platform === value)
                          if (template) {
                        ...
                        <SelectTrigger className='w-full'>
                          <SelectValue>
                            <div className="flex items-center space-x-2">
                              ...
                              <span>{selectedTemplate?.label || 'Selecionar'}</span>
                            </div>
                          </SelectValue>
                        </SelectTrigger>
                        <SelectContent className='h-full'>
                          {linksTemplates.map((template) => (
                            <SelectItem key={template.platform} value={template.platform} className='w-32'>
        ...
        <div className="space-y-6">
          <Card className="sticky top-6">
            <CardHeader>
              ...
              <CardDescription>
                Veja como sua biografia ficará
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    {fields.map((field, index) => (
                      <div key={field.id} className="text-xs text-muted-foreground">
                        {watch(`links.${index}.label`) || 'Nome do link'}: {watch(`links.${index}.url`) || 'URL'}
                      </div>
                    ))}
                  </div>
                )}
