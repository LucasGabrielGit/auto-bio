        ...
        return (
            <div className="min-h-screen bg-linear-to-br from-red-50 to-pink-100 flex items-center justify-center">
                <div className="text-center max-w-md mx-auto p-6">
                    ...
                    <h1 className="text-2xl font-bold text-gray-800 mb-2">Biografia não encontrada</h1>
                    <p className="text-gray-600 mb-6">
                        A biografia que você está procurando não existe ou não está mais disponível.
                    </p>
                    <a
                        href="/"
...
const DefaultTemplate = ({ bio, getPlatformIcon }: { bio: Bio, getPlatformIcon: (label: string) => JSX.Element }) => {
    ...
    return (
        <div className={`min-h-screen ${getStyleClasses(bio.style)}`}>
                ...
                >
                    <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl p-8 mb-8">
                        <div className="flex flex-col justify-center items-center">
                            <h1 className="text-4xl font-bold mb-1 text-center">{bio.title}</h1>
                            <div className="text-sm text-gray-500">
                                Criado por {bio.user?.name || 'Usuário'} em{' '}
                                {new Date(bio.createdAt).toLocaleDateString()}
                            </div>
                        </div>
                        <Separator orientation="horizontal" className="my-6 bg-primary-foreground" />
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {bio.links.map((link) => (
                                <a
                                    ...
                                    className="flex items-center p-4 bg-linear-to-r from-indigo-500 to-purple-600 text-white rounded-xl hover:from-indigo-600 hover:to-purple-700 transition-all duration-300 transform hover:scale-105 shadow-lg"
                                >
